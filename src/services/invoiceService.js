...
const renderInvoiceToBuffer = (invoice) => {
    return new Promise((resolve, reject) => {
        ...
        // ──────────────── FOOTER ────────────────
        // Keep the footer inside PDFKit's printable area. Drawing its text below
        // the configured bottom margin makes PDFKit add a continuation page for
        // each text call, which previously produced two blank pages.
        const footerRuleY = doc.page.height - M - 20;
        const footerTextY = footerRuleY + 10;
        doc.moveTo(M, footerRuleY).lineTo(PAGE_W - M, footerRuleY).strokeColor(BORDER).lineWidth(0.8).stroke();
        doc.fillColor(GRAY).fontSize(7).font('Helvetica')
            .text('Generated by Assured Rewards Billing Engine', M, footerTextY, { width: CONTENT_W / 2 })
            .text('Thank you for choosing Assured Rewards', M + CONTENT_W / 2, footerTextY, { width: CONTENT_W / 2, align: 'right' });
